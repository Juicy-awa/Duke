package com.catclient.duke.utils.skia;
...
import com.catclient.duke.utils.skia.image.ImageHelper;
...
public class Skia {
    ...
    public static void drawBlur(float x, float y, float width, float height) {
        ...
        if (Duke.getInstance().getModuleManager().getModule(HUD.class).blur.get()) {
            ...
            drawImage(KawaseBlur.INGAME_BLUR.getTexture(), 0, 0, window.getWidth(), window.getHeight(), 1F,
    ...
    public static void drawPlayerHead(AbstractClientPlayer player, float x, float y, float width, float height, float radius) {
        ...
        if (imageHelper.load(skiaPath)) {
            ...
            save();
            getCanvas().clipPath(path, ClipMode.INTERSECT, true);
            getCanvas().drawImageRect(imageHelper.get(skiaPath), srcRect, dstRect, null, false);
            getCanvas().drawImageRect(imageHelper.get(skiaPath), srcRect1, dstRect, null, false);
            restore();
        }
    }
